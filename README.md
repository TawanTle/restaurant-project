...
เว็บตัวอย่างใช้ Python มาตรฐานกับ SQLite เปิดเครื่องแล้วรันได้โดยไม่ต้องติดตั้งแพ็กเกจเพิ่ม

## วิธีเปิดเว็บ

1. ติดตั้ง Python 3.10 ขึ้นไป
2. เปิด Terminal ในโฟลเดอร์นี้ แล้วรัน `python app.py` (Windows อาจใช้ `py app.py`)
3. เปิด `http://127.0.0.1:8000` ในเบราว์เซอร์
4. โปรแกรมสร้างไฟล์ `restaurant.db` และเพิ่มข้อมูลตัวอย่างเมื่อเปิดครั้งแรก หากต้องการเริ่มใหม่ให้ปิดโปรแกรมแล้วลบไฟล์ `restaurant.db`
